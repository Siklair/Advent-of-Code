...
fn main() {
    ...
    for (num, init_beam) in init_beams.iter().enumerate() {
        res = res.max(calc_energized(*init_beam, &map, rows, cols));
...
fn calc_energized(init_beam: Beam, map: &HashMap<(usize, usize), char>, rows: usize, cols: usize) -> usize {
